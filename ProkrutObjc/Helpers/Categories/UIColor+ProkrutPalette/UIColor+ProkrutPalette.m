//
//  UIColor+ProkrutPalette.m
//  ProkrutObjc
//
//  Created by Egor Tolstoy on 27/10/16.
//  Copyright © 2016 DevAlloy. All rights reserved.
//

#import "UIColor+ProkrutPalette.h"

@implementation UIColor (ProkrutPalette)

+ (UIColor *)prokrutGreenColor {
    return [UIColor colorWithRed:68.0f/255.0f green:219.0f/255.0f blue:94.0f/255.0f alpha:1.0f];
}

+ (UIColor *)prokrutRedColor {
    return [UIColor colorWithRed:254.0f/255.0f green:63.0f/255.0f blue:53.0f/255.0f alpha:1.0f];
}

@end
